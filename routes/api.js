...
// Category Items Route //
    ...
    router.get('/generate-pdf', async (req, res) => {
        ...
        pdf.text(`Name: ${data.customername}`, 0.5, 1);
        if(data.mob_number){
          pdf.text(`Contact: ${data.mob_number}`, 0.5, 1.25);  
        }
        pdf.text(`Table: ${data.tableno}`, 0.5, 1.5);
        ...
        pdf.text(invoiceText, pageWidth - invoiceWidth - 0.5, 1);
        
        const dateText = `Date: ${data.date}`;
        const dateWidth = pdf.getTextWidth(dateText);
        pdf.text(dateText, pageWidth - dateWidth - 0.5, 1.25);

        const timeText = `Time: ${data.time}`;
        const timeWidth = pdf.getTextWidth(timeText);
        pdf.text(timeText, pageWidth - timeWidth - 0.5, 1.5)
        ...
        if (Array.isArray(tableData) && tableData.every(row => Array.isArray(row))) {
            ...
            let totalQty = qtyValue.reduce((accumulator, current) => accumulator + current);
              ...
              const totalRow = [
                ...
                { content:`Sub-Total: Rs.${formattedTotal}` , styles: {...rowStyles,},colSpan: 2},
              ];
            try {
                pdf.autoTable({
                    startY: 1.75,
        ...
        res.set("Content-Disposition", `attachment; filename="invoice_${data.invoiceNumber}.pdf"`);
        res.set("Content-Type", "application/pdf");
        res.send(pdfBuffer);
    });
