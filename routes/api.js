...
// Category Items Route //
    ...
    async (req,res)=>{
        ...
        let newMyOrders = new CurrentOrders({customername,name,image,price,qty,owner,created_at,tableno});
    ...
    router.post("/cart/order", async (req,res)=> {
        ...
        for (let item of items){
            ...
            let qty = 1;
            const newMyOrders = new CurrentOrders({customername,name,image,qty,owner,price,created_at,tableno});
            newMyOrders.customerId = res.locals.sessionId;
            newMyOrders.status ="Waiting";
            await newMyOrders.save();
