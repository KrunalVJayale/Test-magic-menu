...
router.get("/tables/orders/:number",isLoggedIn,
    wrapAsync(
        ...
        const customername = (await CurrentOrders.findOne({owner:owner,tableno:number })).customername;
        ...
        res.render("listings/adminBill.ejs", {waitingOrders,confirmedOrders,owner, number,customername, date,time});
}));
...
router.get("/tables/bill/:number",isLoggedIn,
    wrapAsync(
        ...
        let date = new Date(Date.now() ).toString().split(" ").slice(1,4).join("-");
        let time =  new Date(Date.now() + (5.5 * 60 * 60 * 1000)).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
        res.render("listings/billPage.ejs", {confirmedOrders,owner, number,customername, date,time});
}));
    ...
    wrapAsync(
        async(req,res)=> {
            ...
            for (let order of orders){
                ...
                const newHistory = new History({customername,name,price,qty,owner,tableno,created_at,confirmed_at,customerId,status,paid_at});
